...
for i in range(1, 36):
    TargetUrl.append(
        f'https://centercourseware.sflep.com/Pioneer%20College%20English%20Integrated%20Course%201/unit_06/main{i}.html?m-1-6-{i}&nocache=' + str(
            random.random()))
    time.sleep(0.1)
# print(TargetUrl)
i = 1
for url in TargetUrl:
    ...
    #ans = [item.replace(" ", "") for item in ans]
    ...
    if not has_empty_string:
        try:
            ...
            #print(empty)
            if empty:
                with open(f'Answer/{filename}.txt', 'w', encoding='utf-8') as file:
                    for x in tx:
                        try:
                            #print("开始尝试第一部分")
